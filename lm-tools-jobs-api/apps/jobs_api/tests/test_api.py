...
def adzuna_api_cassette_request(func):
    ...
    def innner(*args, **kwargs):
        ...
        with settings.VCR.use_cassette(
                path, filter_query_parameters=filter_args):
            func(*args, **kwargs)
    return innner
...
class TestViews(APITestCase):
    ...
    def test_latest_jobs_unknown_job_centre_label(self):
        res = self.client.get(
            ...
            {'job_centre_label': 'not-a-valid-job-centre'}
        )
    ...
    def test_latest_jobs_by_new_job_centre_label(self):
        res = self.client.get(
            reverse('job_adverts_view'),
            {'job_centre_label': 'sutton', 'limit': 3}
        )
    ...
    @adzuna_api_cassette_request
    def test_top_categories_by_postcode_new_location_matching_a_centre(self):
        res = self.client.get(
            reverse('top_categories_view'),
            {'postcode': 'CR9 2TN', 'limit': 3}
        )
